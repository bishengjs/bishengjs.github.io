...
KISSY.add("dom/selector", ["./selector/parser", "dom/basic"], function(S, require) {
  ...
  var document = S.Env.host.document,
    ...
    caches = {}, isContextXML, uuid = 0,
    subMatchesCache = {}, getAttr = function(el, name) {
      if (isContextXML) {
    ...
    "nth-child": function(el, param) {
      ...
      if (parent) {
        var childNodes = parent.childNodes,
          count = 0,
          child, ret, len = childNodes.length;
        for (; count < len; count++) {
  ...
  function singleMatch(el, match) {
    ...
    var matched = 1,
      ...
      matchSuffixLen, matchSuffixIndex;
    if (match.t === "tag") {
  ...
  function select(str, context, seeds) {
    ...
    for (; groupIndex < groupLen; groupIndex++) {
      ...
      var suffix = group.suffix,
        suffixIndex, suffixLen, seedsIndex, mySeeds = seeds,
        seedsLen, id = null;
      if (!mySeeds) {
